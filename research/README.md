...
以下の記録は「サンドボックスから外部通信できずHTMLを取得できなかった」時点のもの。
その後、実サイトのHTMLがリポジトリ直下の `debug.html` としてコミットされ、
**requests + BeautifulSoup だけで判定可能**であることが確認できた（Playwright は不要）。

判明した構造と判定方法は、リポジトリ直下の [`README.md`](../README.md) を参照。
このディレクトリのスクリプトは、別の日付・別サイトを調べ直すときの道具として残してある。

```bash
python3 analyze_debug.py --file ../debug.html    # 実HTMLの解析結果を見る
```
